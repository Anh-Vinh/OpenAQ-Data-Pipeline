...
def process_sensors(spark_session, base_path, execution_date):
    ...
    sensor_df = sensor_df.dropDuplicates(["param_id"])
    sensor_df = sensor_df.dropna(subset=["param_id"])
    
    sensor_df.write \
        .mode("append") \
        .parquet(f"s3a://silver/openaq/parameter/")
...
if __name__ == "__main__":
    ...
    print(f"Parameters data written to silver layer (MinIO)")